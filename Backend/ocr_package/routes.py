...
from ocr_package.forms import RegistrationForm, LoginForm, CardForm, ValidationError
from ocr_package.model import User, Card
from flask_login import login_user, current_user, logout_user, login_required
from ocr_package import calc
...
def detect(img0, model, imgsz,names):
    ...
    pred = non_max_suppression(pred, conf_thres, iou_thres, classes=classes, agnostic=agnostic_nms) #prediction, conf, iou
...
def warpPerspective(landmarks, img):
    src_pts_1 = np.array([
    ...
    cropped_pic1 = cv2.warpPerspective(img, perspect_1, (max_x, max_y))
    return cropped_pic1
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Compte créé avec succès pour {form.name.data}!', 'success')
...
def savecard():
    ...
    with open('Bank_Card_Results.json', 'r') as myfile:
        ...
        if  card != None:
            message = "Cette carte deja existe. Veuillez en choisir un autre."
            redirect(url_for('ocr'))
            return render_template('ocr.html', message=message)
        else: 
...
#             card = Card(card_number=card_number, card_holder=card_holder, valid_date=valid_date, user_id=current_user.id)
#             db.session.add(card)
#             db.session.commit()
#         flash(f'Carte a été ajouté avec succès !', 'success')
#         # return redirect(url_for('ocr'))
            ...
            for file in final_files:
                ...
                if len(date_boxes) < 1:
                    ...
                    for box in date_boxes:
                        ext = Perspective_img[box[1]: box[3], box[0]: box[2]]
                        cv2.rectangle(extracted_card, (box[0], box[1]), (box[2], box[3]), (255, 0, 0), tl)
                else:
